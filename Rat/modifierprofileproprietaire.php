...
if(!empty($_POST)){
	...
	if($modifier == "form"){
		...
		$req = $DB->query("Select * from users where idpublic = :id", array('id' => $_SESSION['id']));
		$req = $req->fetch();
   ...
   if($_FILES['avatar']['size'] <= $tailleMax) {
      ...
      if(in_array($extensionUpload, $extensionsValides)) {
         ...
         if($resultat) {
            	...
            	header('Location: espace-proprietaire.php?id='.$_SESSION['id']);
         } else {
...
<body>
	<center><h3>Modifier votre profile</h3></center><br>
		<div class="form-group">
			<form method="post" enctype="multipart/form-data">
				...
				<button class="btn btn-primary" type="button" style="background-color: #cc0c0b; border-color: #cc0c0b;" onclick="self.location.href='espace-proprietaire.php'">Mon espace</button>
				<br>
				</center>
			</form>
		</div>

...
if(isset($_GET['id']) AND !empty($_GET['id'])) {
   ...
   $ville = $bdd->prepare('SELECT * FROM villes WHERE id = ?');
   $ville->execute(array($get_id));
   if($ville->rowCount() == 1) {
      $ville = $ville->fetch();
      $nom = $ville['nom'];
      $contenu = $ville['contenu'];
   } else {
      die('Cet ville n\'existe pas !');
   }
} else {
   die('Erreur');
}
        ...
        <div class="container"><a class="logo-bbc" href="#"><img id="logo-bbc" src="assets/img/Logo_BBC_Original.jpg"></a><button data-toggle="collapse" class="navbar-toggler" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span><span class="navbar-toggler-icon"></span></button>
            <div class="collapse navbar-collapse" id="navcol-1">
                <ul class="nav navbar-nav mr-auto">
                    ...
                    <li class="nav-item" role="presentation"><a class="nav-link" href="#text-right">Nos tarifs</a></li>
